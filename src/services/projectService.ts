...
class ProjectService implements IProjectService {
  ...
  async getAll(): Promise<ServiceResponse<Project[]>> {
    try {
      ...
      if (!eventsResult.success || !eventsResult.data) {
        return {
          success: false,
          error: 'Failed to fetch events for project query',
        };
      }
  ...
  async getById(id: string): Promise<ServiceResponse<Project>> {
    try {
      ...
      if (parts.length === 2) {
        ...
        const project = await this.storage.get<Project>(`project:${eventId}:${projectId}`);
  ...
  async getByEvent(eventId: string): Promise<ServiceResponse<Project[]>> {
    try {
      const projects = await this.storage.getAll<Project>(`project:${eventId}:`);
  ...
  async create(
    ...
    try {
      if (!projectData.eventId) {
        return {
          success: false,
          error: 'eventId is required to create a project',
        };
      }
      ...
      await this.storage.set(`project:${newProject.eventId}:${newProject.id}`, newProject);
  ...
  async getByHacker(hackerId: string): Promise<ServiceResponse<Project[]>> {
    try {
      ...
      const hackerProjects = allProjectsResult.data.filter((p) => p.hackerId === hackerId);
